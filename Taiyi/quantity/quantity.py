...
class Quantity:
    ...
    def track(self, global_step):
        if self._should_compute(global_step): # 计算当前步是否要进行操作
            result = self._compute(global_step) # 计算模型输入的2-范数
    ...
    def _save(self, global_step, result):
        ...
        """
        self._output[global_step] = self._apply_save_format(result) # 将结果存到_output[0]中
